...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Backtest a simple long/short market-structure overlay on SPY. "
            "Baseline = always long SPY. "
            "Overlay = long SPY when weekly direction == HH/HL, "
            "short SPY when weekly direction == LH/LL, "
            "flat when weekly direction == TRANSITION."
        )
    )
    parser.add_argument(
        "--spy-file",
...
def load_spy_daily(path: Path) -> pd.DataFrame:
    ...
    return (
        ...
        .rename(columns={"Date": "date", "Adj Close": "spy_adj_close"})
    )
...
def load_weekly_classification(path: Path) -> pd.DataFrame:
    ...
    if not df.loc[df["ticker"] == "SPY"].empty:
...
def build_market_regime_from_sectors(classified_df: pd.DataFrame) -> pd.DataFrame:
    ...
    regime["overlay_position_signal"] = regime["overlay_direction"].map(
        {"HH/HL": 1, "LH/LL": -1, "TRANSITION": 0}
    )
    return regime
...
def merge_daily_with_weekly_signal(
    ...
    # Apply signal with a 1-day lag to avoid same-day lookahead.
    merged["overlay_position"] = (
        merged["overlay_position_signal"]
        .fillna(0)
        .astype(int)
        .shift(1)
        .fillna(0)
...
def annualized_return(equity: pd.Series, periods: int) -> float:
    if periods <= 1:
        return 0.0
    total_return_multiple = float(equity.iloc[-1] / equity.iloc[0])
    years = periods / 252.0
    if years <= 0:
        return 0.0
    return total_return_multiple ** (1.0 / years) - 1.0
...
def build_summary_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    summary_specs = [
        ...
        ("overlay_long_short_by_market_structure", "overlay_strategy_return", "overlay_equity", "overlay_position"),
    ]

    for label, ret_col, eq_col, pos_col in summary_specs:
        total_days = int(len(df))
        long_days = int((df[pos_col] == 1).sum())
        short_days = int((df[pos_col] == -1).sum())
        flat_days = int((df[pos_col] == 0).sum())
        invested_days = int((df[pos_col] != 0).sum())
        ...
        rows.append(
            {
                ...
                "invested_share_pct": round(invested_days / total_days * 100.0, 2),
                "long_days": long_days,
...
def build_position_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    total = int(position_counts["day_count"].sum())
    position_counts["share_pct"] = (
        position_counts["day_count"] / total * 100.0
    ).round(2)
    return position_counts.sort_values("position").reset_index(drop=True)
...
def main() -> None:
    ...
    detail_path = out_dir / "spy_overlay_long_short_backtest_daily.csv"
    regime_path = out_dir / "spy_overlay_long_short_weekly_market_regime.csv"
    summary_path = out_dir / "spy_overlay_long_short_backtest_summary.csv"
    overlay_regime_summary_path = out_dir / "spy_overlay_long_short_regime_summary.csv"
    position_summary_path = out_dir / "spy_overlay_long_short_position_summary.csv"
    ...
    position_summary_df = build_position_summary(equity_df)
    ...
    print("- Overlay is long SPY when sector-majority weekly regime is HH/HL")
    print("- Overlay is short SPY when sector-majority weekly regime is LH/LL")
    print("- Overlay is flat when sector-majority weekly regime is TRANSITION")
    print("- Weekly signal is applied with a 1-day lag to avoid same-day lookahead")
    print("- No transaction costs, borrow costs, dividends-on-short, or slippage are included")
